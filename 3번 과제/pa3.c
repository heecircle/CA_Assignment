...
int load_word(unsigned int addr)
{
    ...
    int offset_bit = log2_discrete(nr_words_per_block * BYTES_PER_WORD);
    int index_bit = log2_discrete(nr_sets);
    ...
    unsigned int tags_cal = addr >> (offset_bit + index_bit);
    unsigned int index_cal = (addr << (tag_bit) ) >> (tag_bit + offset_bit);

    /* cache find */
    for(int j = 0; j< nr_ways; j++)
    {
        if(cache[ index_cal * nr_ways + j ].valid != CB_INVALID)
        {
            if(cache[ index_cal * nr_ways + j ].tag == tags_cal)
            {

                cache[ index_cal * nr_ways + j ].timestamp = cycles;
                return CACHE_HIT;
            }
        }
    }
    ...
    /* cache put when empty way exists */

    int start_index = (int)((addr / (nr_words_per_block * BYTES_PER_WORD)))*(nr_words_per_block * BYTES_PER_WORD);
    ...
    for(int j = 0; j< nr_ways;j++)
    {
        ...
        if(cache[index_cal* nr_ways + j ].valid == CB_INVALID)
        {
            start_index = (int)((addr / (nr_words_per_block * BYTES_PER_WORD)))*(nr_words_per_block * BYTES_PER_WORD);

            cache[index_cal* nr_ways + j].valid = CB_VALID;


            for(int k = 0; k< nr_words_per_block * BYTES_PER_WORD ; k++)
            {
                cache[index_cal* nr_ways + j].data[k] = memory[start_index+k];
            }
            cache[index_cal* nr_ways + j].tag = tags_cal;
            cache[index_cal* nr_ways + j].timestamp = cycles;
            return CACHE_MISS;
        }
    ...
    int minimum_way_num = -1;
    int comp_val = 1000000000;
    /* find LRU way */
    for(int j = 0; j< nr_ways; j++)
    {

        if(cache[ index_cal* nr_ways + j ].timestamp < comp_val)
        {
            comp_val = cache[ index_cal* nr_ways + j ].timestamp;
            minimum_way_num = j;
        }
    }

    cache[index_cal* nr_ways + minimum_way_num].valid = CB_VALID;



    if(cache[ index_cal * nr_ways + minimum_way_num ].dirty == CB_DIRTY)
    {
        cache[index_cal * nr_ways + minimum_way_num].dirty = CB_CLEAN;
        temp_addr = (cache[index_cal * nr_ways + minimum_way_num].tag << (32- tag_bit)) | (index_cal << offset_bit);

        for(int k = 0; k < nr_words_per_block * BYTES_PER_WORD ; k++)
        {
            memory[temp_addr | k] = cache[index_cal * nr_ways + minimum_way_num].data[k];
        }
    }
    cache[index_cal* nr_ways + minimum_way_num].tag = tags_cal;
    cache[index_cal* nr_ways + minimum_way_num].timestamp = cycles;
    for(int k = 0; k< nr_words_per_block * BYTES_PER_WORD ; k++)
    {
        cache[index_cal* nr_ways + minimum_way_num].data[k] = memory[(int)((addr /(nr_words_per_block * BYTES_PER_WORD)))*(nr_words_per_block * BYTES_PER_WORD)+k];
...
int store_word(unsigned int addr, unsigned int data)
{
    ...
    /*if cache에 있으면 cache 바꾸고 dirty bit 로 바꾼다ㅏ음에 cache에만 저장 */
    /*없으면 바꾸고 어케야되냐? 불러와야ㅗ디나 */

    int tag_shift = log2_discrete(nr_sets) + log2_discrete(nr_words_per_block * BYTES_PER_WORD);


    int offset_bit = log2_discrete(nr_words_per_block * BYTES_PER_WORD);
    int index_bit = log2_discrete(nr_sets);
    int tag_bit = 32 - (offset_bit + index_bit);
    unsigned int tags_cal = addr >> tag_shift;
    unsigned int index_cal = (addr << (tag_bit) ) >> (tag_bit + offset_bit);
    int temp_index = ((addr << (32-tag_shift)) >> (32 - tag_shift + log2_discrete(nr_words_per_block * BYTES_PER_WORD)));
    int arr[4] = {};
    arr[0] = (data & 0xff000000) >> 24;
    arr[1] = (data & 0x00ff0000) >> 16;
    arr[2] = (data & 0x0000ff00) >> 8;
    arr[3] = (data & 0x000000ff) >> 0;

    int offset = (addr << (32 - offset_bit)) >> (32 - offset_bit);
    ...
    for(int j = 0; j< nr_ways; j++)
    {
        if(cache[ index_cal * nr_ways + j ].valid != CB_INVALID)
        {
            ...
            {
                printf("exist ! \n");
                cache[temp_index * nr_ways + j].dirty = CB_DIRTY;
                cache[ index_cal * nr_ways + j ].timestamp = cycles;

                for (int i = 0; i < BYTES_PER_WORD; i++)
                {
                    cache[temp_index * nr_ways + j].data[offset + i] = arr[i];
                }
                return CACHE_HIT;
            }
        }
    ...
    cache[index_cal* nr_ways + minimum_way_num].timestamp = cycles;
    cache[index_cal* nr_ways + minimum_way_num].valid = CB_VALID;
    cache[temp_index * nr_ways + minimum_way_num].dirty = CB_DIRTY;

    for (int i = 0; i < BYTES_PER_WORD; i++)
    {
        cache[temp_index * nr_ways + minimum_way_num].data[offset + i] = arr[i];
    }
